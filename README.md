...
# Verificsiton of the Deposit Smart Contract in Dafny
...
> A tree is:
> 1. a **binar** tree if every node that is not a leaf has two children and,
...
> Can we _incrementally_ (and efficiently) compute the Merkle attribute on binary complete trees that represent lists that _grow monotonically_ such as `L`, `L + e1`, `(L + e_1) + e_2` (where _+_ is the append operatoron lists) and so on.
...
To check that the nodes of an `int`-decorated tree stores the value of the previous `diff` attribute, we can use the following predicate: 
...
To compute the updated value of `diff` on `n15` after updsting `n6`, we only need to store the three values of the yellow and purple nodes.
