...
module Trees {
    ...
    /** Binary trees with an index.
     *
     *  Values are in a domain `T`.
     *  
     *  @tparam T   The type of values attached to the nodes.
     *  @param  v   The value associated with a node.
     8  @param  id  A sequence that corresponds to the node id.
     */
    datatype ITree<T> = 
        ...
        decreases root
    {
        match root 
            //  a leaf has height 1
            case ILeaf(_, _) => 1
            //  A node has height 1 + max height of children
    ...
    function indexFrom<T>(r : Tree<T>, p : seq<bit>) : ITree<T> 
        // ensures forall n :: n in nodesIn(indexFrom(r, p)) ==> |n.id| >= |p| 
        decreases r
    {
        match r 
